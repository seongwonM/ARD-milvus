...
from .vector_store_common import _INSERT_BATCH, _SEARCH_CHUNK, _TEXT_MAX_BYTES, _trunc

# bench/retrieval_runner.py가 청크를 얼마나 모아서 한 번에 _insert_with_retry로
# 넘길지 계산할 때 쓰는 예산 — gRPC 기본 메시지 크기 제한(64MB)보다 한참 여유있게
# (고차원 모델에서 batch가 너무 크면 "received message larger than max" 에러가 남).
# StarRocks와 공유하지 않는다 — 같은 바이트 수라도 SQL 텍스트 파싱 비용이 훨씬 커서
# StarRocks BE가 OOMKilled됐다(2026-07 실측, MALLOC_CONF 튜닝으로도 재발 —
# vector_store_common.py 주석 참고).
