...
#
#   1) 쿼리 벡터 캐싱 — 로컬 파일 캐시라 milvus/minio 생애주기와 무관, 1회성
#   2) 테스트 A(기준선): N값마다 [milvus/minio 배포 → corpus 색인 → 테스트 → 정리] 반복
#   3) 테스트 B(동시성 램프): [milvus/minio 배포 → corpus 색인 → 테스트 → 정리] 1회
...
# --recreate 불필요 — 컬렉션 자체가 없는 상태에서 시작).
seed_corpus() {
  echo "----- corpus 색인 -----"
...
echo "===== 1) 쿼리 벡터 캐싱 (milvus/minio와 무관한 로컬 캐시, 1회성) ====="
