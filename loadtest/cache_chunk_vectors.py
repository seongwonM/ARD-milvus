...
def main() -> None:
    ...
    if not args.force and ids_path.exists() and vecs_path.exists():
        ...
        if len(cached_vecs) == len(cached_ids) and set(cached_ids) == set(doc_ids):
            logger.info(f"[스킵] 캐시가 현재 corpus {len(doc_ids):,}건과 완전히 일치 — 재임베딩 생략 (다시 만들려면 --force)")
            return
        logger.info(
            f"[재생성] 캐시가 현재 corpus와 불일치(캐시 {len(cached_ids):,}건 vs 현재 {len(doc_ids):,}건) — 다시 임베딩"
        )
