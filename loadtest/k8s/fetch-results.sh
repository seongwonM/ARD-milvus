#!/usr/bin/env bash
# k8s/fetch-results.ps1과 동일한 목적/구조 — 결과(csv/html/log/report.txt)를 로컬로
...
# 순수 디버그 pod(loadtest/k8s/debug-pod.yaml)를 통해 복사한다. 데이터 자체는 PVC에
# 남아있고, 이 pod는 그걸 읽어오는 통로 역할만 한다 — locust-pod.yaml은 캐싱/색인까지
# 직접 실행하는 파드라 fetch 용도로는 부적절해서 안 씀.
#
# 사용법:
#   ./loadtest/k8s/fetch-results.sh <namespace> [출력디렉토리]
#   ./loadtest/k8s/fetch-results.sh user-x0179564-loadtest
#   ./loadtest/k8s/fetch-results.sh user-x0179564-loadtest results-loadtest
set -euo pipefail
