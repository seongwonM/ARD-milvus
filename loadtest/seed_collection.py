"""corpus(약 65,000개 chunk)를 색인한다 (백엔드 재배포마다 반복 실행).

cache_chunk_vectors.py가 미리 계산해둔 임베딩 캐시를 그대로 삽입한다 — Embedding API를
다시 호출하지 않으므로, baseline/ramp 테스트마다 milvus/minio 또는 starrocks를 통째로
재배포해도 매번 6만 건을 재임베딩하는 비용 없이 벡터 삽입만 반복하면 된다.
(캐시가 없으면 먼저 cache_chunk_vectors.py를 실행해야 함)

VECTOR_BACKEND 환경변수(milvus|starrocks, 기본 milvus)로 대상 백엔드를 고른다 —
캐시된 벡터는 두 백엔드가 동일하게 공유하고, 삽입 대상만 달라진다.
...
사용법:
    ...
    VECTOR_BACKEND=starrocks python -m milvus_migration.loadtest.seed_collection --data-root /data
"""
...
from ..store_factory import build_store, collection_name
...
def main() -> None:
    ...
    if not args.recreate and store.has_collection(name):
        size = store.collection_size(name)
        if size == len(chunk_ids):
            logger.info(f"[스킵] 이미 색인됨({cfg.backend}): {name} ({size:,}건) — 재색인하려면 --recreate")
            return
        logger.info(f"[불일치] 기존 컬렉션 크기({size:,}) != corpus({len(chunk_ids):,}) — 재색인 진행")

    logger.info(f"색인 시작(캐시된 벡터 삽입, backend={cfg.backend}): {len(chunk_ids):,}건 → '{name}'")
