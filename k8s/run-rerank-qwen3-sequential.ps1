<#
.SYNOPSIS
  k8s/rerank-job-qwen3-top{5,10,20,50,100}.yaml을 동시에 띄우지 않고 하나씩 순서대로
  실행합니다: 적용 -> 완료 대기(결과는 Job 안에서 PVC에 저장됨) -> 다음 top_n 적용.

  top_n별로 Job(pod)을 계속 분리하는 이유는 k8s/rerank-job-bge.yaml과 같습니다 — 한 pod에서
  ...
  k8s/fetch-results.ps1이 나중에 로그(kubectl logs job/...)를 읽어올 수 있다.

.EXAMPLE
  ./k8s/run-rerank-qwen3-sequential.ps1
...
foreach ($n in $topNs) {
    ...
    $file = "$PSScriptRoot/rerank-job-qwen3-top$n.yaml"
...
Write-Host "`n전체 완료. 결과는 PVC(bench-results)에 저장돼 있습니다 — ./k8s/fetch-results.ps1 -Stage rerank 로 로컬에 복사하세요."
